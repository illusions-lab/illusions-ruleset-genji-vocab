...
- `genji-out-of-dict` ルールに `suggestsDictionaryEntry: true` を宣言。ホスト（illusions）がこの純データのフラグを読み、検出結果カードと波線の右クリックメニューに「ユーザー辞書に追加」アクションを表示できるようになる。ルールセット側のロジック・挙動に変更はない（マニフェストの宣言のみ）。
