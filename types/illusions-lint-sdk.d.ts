...
declare module "illusions-lint-sdk" {
  ...
  export abstract class AbstractMorphologicalLintRule extends AbstractLintRule {
    /** Provided by the runtime base: morphological rules implement lintWithTokens. */
    lint(text: string, config: LintRuleConfig): LintIssue[];
  ...
  export abstract class AbstractDocumentLintRule extends AbstractLintRule {
    /** Provided by the runtime base: document rules return nothing per-paragraph. */
    lint(text: string, config: LintRuleConfig): LintIssue[];
  ...
  export interface DictToolkit {
    ...
    /**
     * Synchronous membership check against the per-batch prewarmed snapshot.
     *
     * The lint pass is synchronous but dictionary I/O is async, so illusions
     * prewarms membership for the batch on the renderer and installs it before
     * your rule runs. Returns `false` when the dictionary is not ready OR the
     * term was not prewarmed.
     *
     * Prefer `lookupCached` in a rule: a term that was not prewarmed returns
     * `undefined`, letting you SKIP it (never flag) instead of treating an
     * unknown term as absent.
     */
    hasCached(term: string): boolean;
    /** Synchronous projection lookup against the per-batch prewarmed snapshot. */
